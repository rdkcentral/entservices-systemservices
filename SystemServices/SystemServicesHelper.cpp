    ...
    namespace Plugin {
        ...
        string getModel()
        {
            ...
            LOGWARN("%s: opened pipe for command /lib/rdk/getDeviceDetails.sh, with result %s : %s\n",
                    __FUNCTION__ , pipe ? "sucess" : "failure", strerror(errno));
            if (!pipe) {
...
bool getDownloadProgress(int& downloadPercent)
{
    ...
    if (retStatus == true && (!file_content.empty()))
    {
        ...
        {
              retStatus = false;
              // trim the leading and trailing spaces/newlines
              ...
              if (found_DOWN != std::string::npos)
              {
                  ...
                  if (downList.size() >= 3 && !downList[0].empty()
                      && downList[1] == "of" && !downList[2].empty())
                  {
