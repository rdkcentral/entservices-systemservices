...
protected:
    ...
    SystemServicesTest()
        ...
        , _thermalModeChangedNotification(nullptr)
        , _rebootNotification(nullptr)
...
protected:
    ...
    virtual void SetUp() override
    {
        SystemServicesTest::SetUp();
    }
    virtual void TearDown() override
    {
        SystemServicesTest::TearDown();
    }
};
...
protected:
        ...
        : SystemServicesEventTest()
    {
        ON_CALL(*p_iarmBusImplMock, IARM_Bus_RegisterEventHandler(::testing::_, ::testing::_, ::testing::_))
            .WillByDefault(::testing::Invoke(
                [&](const char* ownerName, IARM_EventId_t eventId, IARM_EventHandler_t handler) -> IARM_Result_t {
                    if ((string(IARM_BUS_SYSMGR_NAME) == string(ownerName)) && (eventId == IARM_BUS_SYSMGR_EVENT_SYSTEMSTATE)) {
                        systemStateChanged = handler;
                    }
                    // TODO: Anything to be done for IARM_BUS_SYSMGR_EVENT_DEVICE_UPDATE_RECEIVED ?
                    return IARM_RESULT_SUCCESS;
                }));
        ON_CALL(*p_iarmBusImplMock, IARM_Bus_RegisterCall(::testing::_, ::testing::_))
            .WillByDefault(::testing::Invoke(
                [&](const char* methodName, IARM_BusCall_t handler) -> IARM_Result_t {
    ...
    virtual void TearDown() override
    {
        SystemServicesEventTest::TearDown();
    }
};
...
TEST_F(SystemServicesEventTest, PendingReboot)
{
    ...
    EVENT_SUBSCRIBE(0, _T("onFirmwarePendingReboot"), _T("org.rdk.System"), message);
    ...
    EVENT_UNSUBSCRIBE(0, _T("onFirmwarePendingReboot"), _T("org.rdk.System"), message);
}
...
TEST_F(SystemServicesTest, Firmware)
{
    ...
    file << "Method|xconf \nProto|\nStatus|Failure\nReboot|\nFailureReason|Invalid Request\nDnldVersn|CUSTOM1_5.6p4s1_VBN_sey\nDnldFile|\nDnlURL|https://test.tv:443/Images/CUSTOM1_5.6p4s1_VBN_sey-signed.bin\n FwupdateState|Failed\n";
    ...
    EXPECT_EQ(response, string("{\"currentFWVersion\":\"CUSTOM5_VBN_2203_sprint_20220331225312sdy_NG\",\"downloadedFWVersion\":\"\",\"downloadedFWLocation\":\"\",\"isRebootDeferred\":false,\"success\":true}"));
...
TEST_F(SystemServicesEventTest, Timezone)
{
    ...
    EVENT_SUBSCRIBE(0, _T("onTimeZoneDSTChanged"), _T("org.rdk.System"), message);
    ...
    EVENT_UNSUBSCRIBE(0, _T("onTimeZoneDSTChanged"), _T("org.rdk.System"), message);
}
...
TEST_F(SystemServicesEventTest, ValidTerritory)
{
    ...
    EVENT_SUBSCRIBE(0, _T("onTerritoryChanged"), _T("org.rdk.System"), message);
    ...
    EVENT_UNSUBSCRIBE(0, _T("onTerritoryChanged"), _T("org.rdk.System"), message);
}
...
TEST_F(SystemServicesTest, rebootReason)
{
    ...
    file.open("/opt/secure/reboot/previousreboot.info");
    ...
    file.open("/opt/secure/reboot/hardpower.info");
...
TEST_F(SystemServicesTest, SystemVersions)
{
    ...
    EXPECT_EQ(response, string("{\"stbVersion\":\"CUSTOM5_VBN_2203_sprint_20220331225312sdy_NG\",\"receiverVersion\":\"000.36.0.0\",\"stbTimestamp\":\"Fri 05 Aug 2022 16:14:54 UTC\",\"success\":true}"));
}
/*******************************************************************************************************************
    ...
    EXPECT_CALL(*p_wrapsImplMock, popen(::testing::_, ::testing::_))
        ...
        .WillRepeatedly(::testing::Invoke(
            [&](const char* command, const char* type) {
                EXPECT_EQ(string(command), string(_T("/usr/bin/rdkvfwupgrader 0 4 >> /opt/logs/swupdate.log &")));
                return nullptr;
            }));
...
TEST_F(SystemServicesTest, setDeepSleepTimer)
{
    ...
    EXPECT_CALL(PowerManagerMock::Mock(), SetDeepSleepTimer(::testing::_))
        .WillOnce([](const int timeOut) -> uint32_t {
            EXPECT_EQ(timeOut, 5);
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonFailure)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_IR)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_IR;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_RCU_BT)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_BLUETOOTH;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_RCU_RF4CE)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_RF4CE;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_GPIO)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_GPIO;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_LAN)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_LAN;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_WLAN)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_WIFI;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_TIMER)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_TIMER;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_FRONT_PANEL)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_FRONTPANEL;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_WATCHDOG)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_WATCHDOG;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_SOFTWARE_RESET)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_SOFTWARERESET;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_THERMAL_RESET)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_THERMALRESET;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_WARM_RESET)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_WARMRESET;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_COLDBOOT)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_COLDBOOT;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_STR_AUTH_FAILURE)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_STRAUTHFAIL;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_CEC)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_CEC;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_PRESENCE)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_PRESENCE;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getWakeupReasonSuccess_When_WAKEUPREASON_VOICE)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetLastWakeupReason(::testing::_))
        .WillOnce([](WakeupReason& reason) -> uint32_t {
            reason = WakeupReason::WAKEUP_REASON_VOICE;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesEventIarmTest, onFirmwareUpdateStateChange)
{
    ...
    EVENT_SUBSCRIBE(0, _T("onFirmwareUpdateStateChange"), _T("org.rdk.System"), message);
    ...
    EVENT_UNSUBSCRIBE(0, _T("onFirmwareUpdateStateChange"), _T("org.rdk.System"), message);
}
...
TEST_F(SystemServicesEventIarmTest, onSystemClockSet)
{
    ...
    EVENT_SUBSCRIBE(0, _T("onSystemClockSet"), _T("org.rdk.System"), message);
...
TEST_F(SystemServicesTest, getCoreTemperature)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetThermalState(::testing::_))
        .WillOnce([](float& currentTemperature) -> uint32_t {
...
TEST_F(SystemServicesTest, getTemperatureThresholds)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetTemperatureThresholds(::testing::_, ::testing::_))
        .WillOnce([](float& high, float& critical) -> uint32_t {
            high = 100;
            critical = 200;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getOvertempGraceInterval)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetOvertempGraceInterval(::testing::_))
        .WillOnce([](int& graceInterval) -> uint32_t {
            graceInterval = 600;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getPowerState)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetPowerState(::testing::_, ::testing::_))
        .WillOnce([](PowerState& currentState, PowerState& previousState) -> uint32_t {
...
TEST_F(SystemServicesTest, setPowerStateFailed_when_Bus_call_fails)
{
    EXPECT_CALL(PowerManagerMock::Mock(), SetPowerState(::testing::_, ::testing::_, ::testing::_))
        .WillOnce([](const int keyCode, const PowerState powerState, const string& reason) -> uint32_t {
            EXPECT_EQ(powerState, PowerState::POWER_STATE_ON);
            return Core::ERROR_GENERAL;
        });
 ...
 *        then setPowerState will sets with STANDBY state and setPowerState shall be succeeded.
 *
...
TEST_F(SystemServicesTest, setPowerStateSuccess_when_powerstate_Light_sleep)
{
    EXPECT_CALL(PowerManagerMock::Mock(), SetPowerState(::testing::_, ::testing::_, ::testing::_))
        .WillOnce([](const int keyCode, const PowerState powerState, const string& reason) -> uint32_t {
            EXPECT_EQ(powerState, PowerState::POWER_STATE_STANDBY);
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesTest, getPowerStateBeforeReboot)
{
    EXPECT_CALL(PowerManagerMock::Mock(), GetPowerStateBeforeReboot(::testing::_))
        .WillOnce([](PowerState& powerStateBeforeReboot) -> uint32_t {
            powerStateBeforeReboot = PowerState::POWER_STATE_STANDBY_DEEP_SLEEP;
            return Core::ERROR_NONE;
        });
...
TEST_F(SystemServicesEventIarmTest, onSystemPowerStateChanged_From_DEEPSLEEP_to_ON)
{
    ...
    EVENT_UNSUBSCRIBE(0, _T("onSystemPowerStateChanged"), _T("org.rdk.System"), message);
}
...
TEST_F(SystemServicesEventIarmTest, onSystemPowerStateChanged_PowerState_ON_To_DEEPSLEEP)
{
    ...
    dcmPropertiesFile << "LOG_SERVER=test.tv\n";
...
TEST_F(SystemServicesEventIarmTest, onNetworkStandbyModeChanged)
{
    ...
    EVENT_UNSUBSCRIBE(0, _T("onNetworkStandbyModeChanged"), _T("org.rdk.System"), message);
}
...
TEST_F(SystemServicesEventIarmTest, onRebootRequest)
{
        ...
        .WillOnce(::testing::Invoke(
            [&](const uint32_t, const Core::ProxyType<Core::JSON::IElement>& json) {
                ...
                EXPECT_THAT(text, ::testing::MatchesRegex(_T("\\{"
                                                             ...
                                                             "\"params\":"
                                                             "\\{"
                                                             "\"requestedApp\":\"requestorL1test\","
                                                             "\"rebootReason\":\"reasonOther\""
                                                             "\\}"
                                                             "\\}")));
    ...
    EVENT_UNSUBSCRIBE(0, _T("onRebootRequest"), _T("org.rdk.System"), message);
}
 ...
 *                                     "friendly_id":"IP061-ec","imageVersion":"CUSTOM7_VBN_23Q1_sprint_20230129224229sdy_SYNA_CI",
 *                                     "version":"CUSTOM7_VBN_23Q1_sprint_20230129224229sdy_SYNA_CI",
 *                                     "software_version":"CUSTOM7_VBN_23Q1_sprint_20230129224229sdy_SYNA_CI",
 *                                     "model_number":"CUSTOM7","wifi_mac":"D4:52:EE:32:A3:B1","success":true}
 */
TEST_F(SystemServicesTest, getDeviceInfoSuccess_onQueryParameterHasNoLabelParam)
{
    ...
    propFile.close();
     ...
     EXPECT_EQ(response, _T("{\"make\":\"SKY\",\"bluetooth_mac\":\"D4:52:EE:32:A3:B2\",\"boxIP\":\"192.168.1.0\",\"build_type\":\"VBN\",\"estb_mac\":\"D4:52:EE:32:A3:B0\",\"eth_mac\":\"D4:52:EE:32:A3:B0\",\"friendly_id\":\"\",\"imageVersion\":\"CUSTOM7_VBN_23Q1_sprint_20230129224229sdy_SYNA_CI\",\"version\":\"CUSTOM7_VBN_23Q1_sprint_20230129224229sdy_SYNA_CI\",\"software_version\":\"CUSTOM7_VBN_23Q1_sprint_20230129224229sdy_SYNA_CI\",\"model_number\":\"CUSTOM7\",\"wifi_mac\":\"D4:52:EE:32:A3:B1\",\"success\":true}"));
     file.Destroy();
}
...
TEST_F(SystemServicesTest, requestSystemRebootSuccess_NrdPluginNotRunning)
{
    ...
    EXPECT_CALL(*p_readprocImplMock, readproc(dummyProcTab, ::testing::_))
        .WillOnce([&proc_info](PROCTAB* pt, proc_t* p) -> proc_t* {
            std::string input_pname = "unrelated_process";
    ...
    EXPECT_CALL(PowerManagerMock::Mock(), Reboot(::testing::_, ::testing::_, ::testing::_))
        .WillOnce(
            [](const string& rebootRequestor, const string& rebootReasonCustom, const string& rebootReasonOther) -> uint32_t {
                EXPECT_EQ(rebootRequestor, "SystemServices");
                EXPECT_EQ(rebootReasonCustom, "l1Trigger");
                EXPECT_EQ(rebootReasonOther, "l1Trigger");
                return Core::ERROR_NONE;
            });
...
TEST_F(SystemServicesTest, requestSystemRebootSuccess_onRebootBusAPIFailed)
{
    ...
    EXPECT_CALL(PowerManagerMock::Mock(), Reboot(::testing::_, ::testing::_, ::testing::_))
        .WillOnce(
            [](const string& rebootRequestor, const string& rebootReasonCustom, const string& rebootReasonOther) -> uint32_t {
                EXPECT_EQ(rebootRequestor, "SystemServices");
                EXPECT_EQ(rebootReasonCustom, "FIRMWARE_FAILURE");
                EXPECT_EQ(rebootReasonOther, "FIRMWARE_FAILURE");
                return Core::ERROR_ILLEGAL_STATE;
            });
    EXPECT_EQ(Core::ERROR_NONE, handler.Invoke(connection, _T("reboot"), _T("{\"rebootReason\":\"FIRMWARE_FAILURE\"}"), response));
    EXPECT_EQ(response, string("{\"IARM_Bus_Call_STATUS\":5,\"success\":true}"));
}
 ...
 */
TEST_F(SystemServicesTest, getStateInfoSuccess_onQueryParamCardCUSTOM3Status)
{
    /*sets the expectation that IARM_Bus_Call should not be called*/
    EXPECT_CALL(*p_iarmBusImplMock, IARM_Bus_Call)
        .Times(0);
...
TEST_F(SystemServicesEventTest, onMacAddressesRetrieved)
{
    ...
    ON_CALL(*p_wrapsImplMock, v_secure_popen(::testing::_, ::testing::_, ::testing::_))
        .WillByDefault(::testing::Invoke(
            [&](const char* type, const char* command, va_list args) -> FILE* {
                ...
                vsnprintf(strFmt, sizeof(strFmt), command, args2);
                va_end(args2);
                if (strcmp(strFmt, "/lib/rdk/getDeviceDetails.sh read ecm_mac") == 0) {
                    EXPECT_EQ(string(strFmt), string(_T("/lib/rdk/getDeviceDetails.sh read ecm_mac")));
                    valueToReturn = "A8:11:XX:FD:0C:XX";
                } else if (strcmp(strFmt, "/lib/rdk/getDeviceDetails.sh read estb_mac") == 0) {
                    EXPECT_EQ(string(strFmt), string(_T("/lib/rdk/getDeviceDetails.sh read estb_mac")));
                    valueToReturn = "A8:11:XX:FD:0C:XX";
                } else if (strcmp(strFmt, "/lib/rdk/getDeviceDetails.sh read moca_mac") == 0) {
                    EXPECT_EQ(string(strFmt), string(_T("/lib/rdk/getDeviceDetails.sh read moca_mac")));
                    valueToReturn = "00:15:5F:XX:20:5E:57:XX";
                } else if (strcmp(strFmt, "/lib/rdk/getDeviceDetails.sh read eth_mac") == 0) {
                    EXPECT_EQ(string(strFmt), string(_T("/lib/rdk/getDeviceDetails.sh read eth_mac")));
                    valueToReturn = "A8:11:XX:FD:0C:XX";
                } else if (strcmp(strFmt, "/lib/rdk/getDeviceDetails.sh read wifi_mac") == 0) {
                    EXPECT_EQ(string(strFmt), string(_T("/lib/rdk/getDeviceDetails.sh read wifi_mac")));
                    valueToReturn = "A8:11:XX:FD:0C:XX";
    ...
    EVENT_SUBSCRIBE(0, _T("onMacAddressesRetreived"), _T("org.rdk.System"), message);
    ...
    EVENT_UNSUBSCRIBE(0, _T("onMacAddressesRetreived"), _T("org.rdk.System"), message);
    file.Destroy();
}
...
TEST_F(SystemServicesEventTest, onFirmwareUpdateInfoReceived_WithHttpStatusCode460)
{
    ...
    fileVer << "imagename:CUSTOM5_VBN_2203_sprint_20220331225312sdy_NG";
    fileVer.close();

    std::ofstream http_code_str("/tmp/xconf_httpcode_thunder.txt");
    http_code_str << "460";
    http_code_str.close();

    std::ofstream response_str("/tmp/xconf_response_thunder.txt");
    response_str << "{\"firmwareVersion\":\"1234\"}";
    response_str.close();
    ...
    EVENT_SUBSCRIBE(0, _T("onFirmwareUpdateInfoReceived"), _T("org.rdk.System"), message);
    ...
    EVENT_UNSUBSCRIBE(0, _T("onFirmwareUpdateInfoReceived"), _T("org.rdk.System"), message);

    // Clear file contents
    fileVer.open("/version.txt", std::ofstream::out | std::ofstream::trunc);
    fileVer.close();
    http_code_str.open("/tmp/xconf_httpcode_thunder.txt", std::ofstream::out | std::ofstream::trunc);
    http_code_str.close();
    response_str.open("/tmp/xconf_response_thunder.txt", std::ofstream::out | std::ofstream::trunc);
    response_str.close();
}
...
TEST_F(SystemServicesEventTest, onFirmwareUpdateInfoReceived_WhenEnvPROD)
{
    Core::Event onFirmwareUpdateInfoReceived(false, true);
    std::ofstream fileVer("/version.txt");
    fileVer << "imagename:CUSTOM5_PROD_2203_sprint_20220331225312sdy_NG";
    fileVer.close();
 ...
 *
 * @param[in]   :  This method takes no parameters.
 * @return      :  {\"xconfParams\":{\"env\":\"dev\",\"eStbMac\":\"\",\"model\":\"ERROR\",\"firmwareVersion\":\"CUSTOM5_2203_sprint_20220331225312sdy_NG\"},\"success\":true}
 */
TEST_F(SystemServicesTest, getXconfParamsSuccess_whenVersionStringWithoutEnvInfo)
{
    ofstream file("/version.txt");
    file << "imagename:CUSTOM5_2203_sprint_20220331225312sdy_NG";
    file.close();
    ...
    EXPECT_EQ(Core::ERROR_NONE, handler.Invoke(connection, _T("getXconfParams"), _T("{}"), response));
    EXPECT_EQ(response, string("{\"xconfParams\":{\"env\":\"dev\",\"eStbMac\":\"D4:52:EE:32:A3:B0\",\"model\":\"ERROR\",\"firmwareVersion\":\"CUSTOM5_2203_sprint_20220331225312sdy_NG\"},\"success\":true}"));
}
 ...
 *
 * @param[in]   :  This method takes no parameters.
 * @return      :  "{\"xconfParams\":{\"env\":\"vbn\",\"eStbMac\":\"D4:52:EE:32:A3:B0\",\"model\":\"CUSTOM2\",\"firmwareVersion\":\"CUSTOM5_VBN_2203_sprint_20220331225312sdy_NG\"},\"success\":true}"
 */
TEST_F(SystemServicesTest, getXconfParamsSuccess_withAllXConfparams)
{
    ...
    EXPECT_EQ(Core::ERROR_NONE, handler.Invoke(connection, _T("getXconfParams"), _T("{}"), response));
    EXPECT_EQ(response, string("{\"xconfParams\":{\"env\":\"vbn\",\"eStbMac\":\"D4:52:EE:32:A3:B0\",\"model\":\"CUSTOM2\",\"firmwareVersion\":\"CUSTOM5_VBN_2203_sprint_20220331225312sdy_NG\"},\"success\":true}"));
}
/**
 ...
 *
 * @param[in]   :  This method takes no parameters.
 * @return      :  {\"xconfParams\":{\"env\":\"vbn\",\"eStbMac\":\"D4:52:EE:32:A3:B0\",\"model\":\"ERROR\",\"firmwareVersion\":\"CUSTOM5_VBN_2203_sprint_20220331225312sdy_NG\"},\"success\":true}
 */
TEST_F(SystemServicesTest, getXconfParamsSuccess_onFirmvalueVBN)
{
    ...
    EXPECT_EQ(Core::ERROR_NONE, handler.Invoke(connection, _T("getXconfParams"), _T("{}"), response));
    EXPECT_EQ(response, string("{\"xconfParams\":{\"env\":\"vbn\",\"eStbMac\":\"D4:52:EE:32:A3:B0\",\"model\":\"ERROR\",\"firmwareVersion\":\"CUSTOM5_VBN_2203_sprint_20220331225312sdy_NG\"},\"success\":true}"));
}
 ...
 *
 * @param[in]   :  This method takes no parameters.
 * @return      :  {\"xconfParams\":{\"env\":\"prod\",\"eStbMac\":\"D4:52:EE:32:A3:B0\",\"model\":\"ERROR\",\"firmwareVersion\":\"CUSTOM5_PROD_2203_sprint_20220331225312sdy_NG\"},\"success\":true}
 */
TEST_F(SystemServicesTest, getXconfParamsSuccess_onFirmvaluePROD)
{
    ofstream file("/version.txt");
    file << "imagename:CUSTOM5_PROD_2203_sprint_20220331225312sdy_NG";
    file.close();
    // estb_mac information
    file.open("/tmp/.estb_mac");
    file << "D4:52:EE:32:A3:B0";
    ...
    EXPECT_EQ(Core::ERROR_NONE, handler.Invoke(connection, _T("getXconfParams"), _T("{}"), response));
    EXPECT_EQ(response, string("{\"xconfParams\":{\"env\":\"prod\",\"eStbMac\":\"D4:52:EE:32:A3:B0\",\"model\":\"ERROR\",\"firmwareVersion\":\"CUSTOM5_PROD_2203_sprint_20220331225312sdy_NG\"},\"success\":true}"));
}
 ...
 *
 * @param[in]   :  This method takes no parameters.
 * @return      :  {\"xconfParams\":{\"env\":\"qa\",\"eStbMac\":\"D4:52:EE:32:A3:B0\",\"model\":\"ERROR\",\"firmwareVersion\":\"CUSTOM5_QA_2203_sprint_20220331225312sdy_NG\"},\"success\":true}
 */
TEST_F(SystemServicesTest, getXconfParamsSuccess_onFirmvalueQA)
{
    ofstream file("/version.txt");
    file << "imagename:CUSTOM5_QA_2203_sprint_20220331225312sdy_NG";
    file.close();
    // estb_mac information
    file.open("/tmp/.estb_mac");
    file << "D4:52:EE:32:A3:B0";
    ...
    EXPECT_EQ(Core::ERROR_NONE, handler.Invoke(connection, _T("getXconfParams"), _T("{}"), response));
    EXPECT_EQ(response, string("{\"xconfParams\":{\"env\":\"qa\",\"eStbMac\":\"D4:52:EE:32:A3:B0\",\"model\":\"ERROR\",\"firmwareVersion\":\"CUSTOM5_QA_2203_sprint_20220331225312sdy_NG\"},\"success\":true}"));
}
    ...
    ON_CALL(*p_iarmBusImplMock, IARM_Bus_Call)
        .WillByDefault(
            [](const char* ownerName, const char* methodName, void* arg, size_t argLen) {
                ...
                param->bufLen = strlen(str) + 1;
                strncpy(param->buffer, str, param->bufLen - 1);
                param->buffer[param->bufLen - 1] = '\0';
...
#else
/**
 * @brief : getSerialNumber when Bus call for retrieving the serial number failed.
 *        Check if BUS call to retrieve the serial number is failed,
 *        then getSerialNumber shall be failed and returns an error message in the response.
 *
 * @param[in]   :  This method takes no parameters.
 * @return      :  {"success":false}
 */
TEST_F(SystemServicesTest, getSerialNumberSnmpFailed_whenBusCallFailed)
{
    ON_CALL(*p_iarmBusImplMock, IARM_Bus_Call)
        .WillByDefault(
            [](const char* ownerName, const char* methodName, void* arg, size_t argLen) {
...
/**
 * @brief : getSerialNumber when Bus call for retrieving the serial number succeeded.
 *        Check if BUS call to retrieve the serial number succeeded,
 *        then getSerialNumber shall be succeeded and
 *        returns retrieved serial number in the response.
 *
 * @param[in]   :  This method takes no parameters.
 * @return      :  {"serialNumber":"32E10400103240447","success":true}
 */
TEST_F(SystemServicesTest, getSerialNumberSnmpSuccess_whenBusCallSuccess)
{
    ON_CALL(*p_iarmBusImplMock, IARM_Bus_Call)
        .WillByDefault(
            [](const char* ownerName, const char* methodName, void* arg, size_t argLen) {
                ...
                EXPECT_EQ(string(methodName), string(_T(IARM_BUS_MFRLIB_API_GetSerializedData)));
                auto* param = static_cast<IARM_Bus_MFRLib_GetSerializedData_Param_t*>(arg);
                const char* str = "32E10400103240447";
                param->bufLen = strlen(str) + 1;
                strncpy(param->buffer, str, param->bufLen - 1);
                param->buffer[param->bufLen - 1] = '\0';
...
TEST_F(SystemServicesEventIarmTest, onSystemModeChanged)
{
    ...
    EVENT_UNSUBSCRIBE(0, _T("onSystemModeChanged"), _T("org.rdk.System"), message);
}
...
/**
 * @brief : uploadLogsAsync  when /usr/bin/logupload not present.
 *          Checks logupload is not present in /usr/bin, then uploadLogsAsync
...
TEST_F(SystemServicesTest, uploadLogsAsyncSuccess_WithBuildTypeProd)
{
    ...
    ofstream dcmPropertiesFile("/etc/dcm.properties");
    dcmPropertiesFile << "LOG_SERVER=test.tv\n";
    dcmPropertiesFile << "DCM_LOG_SERVER=test.tv\n";
    dcmPropertiesFile << "DCM_LOG_SERVER_URL=https://test.tv/loguploader/getSettings\n";
    dcmPropertiesFile << "DCM_SCP_SERVER=test.tv\n";
    dcmPropertiesFile << "HTTP_UPLOAD_LINK=https://test.tv/cgi-bin/S3.cgi\n";
    dcmPropertiesFile << "DCA_UPLOAD_URL=https://test.tv\n";
    dcmPropertiesFile.close();
...
* Note:
* input_ppid value is dynamic and unpredictable within the test environment,Hence simulate a condition where proc_info.ppid (the Parent Process ID)
* matches input_ppid  directly is challengig.
* Therefore, we have not included a test case for successfully aborting the log upload when child processes are found.
* However, included success scenario for aborting the log upload when no child processes are found.
**
********************************************************************************************************************/

/**
 * @brief : abortLogUploadFailure when No active upload log process is running
...
TEST_F(SystemServicesTest, abortLogUploadFailure_whenNoActiveUploadLog)
{
    EXPECT_EQ(Core::ERROR_GENERAL, handler.Invoke(connection, _T("abortLogUpload"), _T("{}"), response));
}

/**
 * @brief : abortLogUploadSuccess_OpenprocFailedOnGettingChildProcessId
 *          Checks if the abortLogUpload method is called when openProc failed during the retrieval of child process
...
/**
 * @brief :  abortLogUpload_ReadprocFailedOnGettingChildProcessId
 *          Checks if the abortLogUpload method is called when readProc failed during the retrieval of child process
 *          returns a response status as true.
 *
 * @param[in]   :  no parameter
 * @return      :  "{\"success\":true}")
...
{
    PROCTAB* dummyProcTab = reinterpret_cast<PROCTAB*>(1);
    const string uploadStbLogFile = _T("/lib/rdk//usr/bin/logupload");
    Core::File file(uploadStbLogFile);
    file.Create();
    EXPECT_TRUE(Core::File(string(_T("/usr/bin/logupload"))).Exists());
...
TEST_F(SystemServicesEventIarmTest, onLogUploadSuccess_withUploadStatusSuccess)
{
    ...
    EVENT_SUBSCRIBE(0, _T("onLogUpload"), _T("org.rdk.System"), message);
    ...
    EVENT_UNSUBSCRIBE(0, _T("onLogUpload"), _T("org.rdk.System"), message);
}
...
TEST_F(SystemServicesEventIarmTest, onBlocklistChanged)
{
    ...
    EVENT_SUBSCRIBE(0, _T("onBlocklistChanged"), _T("org.rdk.System"), message);
    ...
    EVENT_UNSUBSCRIBE(0, _T("onBlocklistChanged"), _T("org.rdk.System"), message);
}
